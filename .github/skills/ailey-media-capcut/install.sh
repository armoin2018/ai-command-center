...
# CapCut AI-ley Skill Installation Script
# Automates setup, verification, and build of the CapCut skill
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║         CapCut AI-ley Skill Installation Script           ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [[ ! -f "package.json" ]]; then
  ...
  echo -e "${YELLOW}  Please run this script from the CapCut skill directory${NC}"
  exit 1
fi
...
if [[ ! -f ".env" ]]; then
  ...
  echo ""
  echo -e "${BLUE}  ⚠ Please configure .env with your CapCut API credentials:${NC}"
  echo -e "${BLUE}    - CAPCUT_API_KEY${NC}"
  echo -e "${BLUE}    - CAPCUT_API_SECRET${NC}"
  echo -e "${BLUE}    - CAPCUT_ACCESS_TOKEN${NC}"
  echo -e "${BLUE}    - CAPCUT_USER_ID${NC}"
  echo ""
  echo -e "${BLUE}  Get credentials from: https://developers.capcut.com/${NC}"
else
...
echo "4. Start creating: ${YELLOW}npm run project -- create --name 'My Video'${NC}"
echo ""
...
echo "  ${YELLOW}npm run project${NC}      - Project management"
echo "  ${YELLOW}npm run video${NC}        - Video editing"
echo "  ${YELLOW}npm run asset${NC}        - Asset management"
echo "  ${YELLOW}npm run template${NC}     - Template operations"
echo "  ${YELLOW}npm run export${NC}       - Export/render video"
...
echo "  • Developer Portal: https://developers.capcut.com/"
echo ""
echo -e "${BLUE}For more help, see README.md or run: npm run help${NC}"
echo ""
