#!/usr/bin/env node
/**
 * Cross-platform setup script for aicc-php-developer
...
if (ws) {
  ...
  const agentSrc = path.join(PLUGIN_ROOT, 'agents', 'ailey-php-developer.agent.md');
  const agentDest = path.join(ws, '.github', 'agents', 'ailey-php-developer.agent.md');
  ...
  if (total > 0) console.log(`  ✅ Installed ${total} PHP Developer resource(s) into workspace`);
} else {
