#!/usr/bin/env node
/**
 * Cross-platform setup script for aicc-css-developer
...
if (ws) {
  ...
  const agentSrc = path.join(PLUGIN_ROOT, 'agents', 'ailey-css-developer.agent.md');
  const agentDest = path.join(ws, '.github', 'agents', 'ailey-css-developer.agent.md');
  ...
  if (total > 0) console.log(`  ✅ Installed ${total} CSS Developer resource(s) into workspace`);
} else {
