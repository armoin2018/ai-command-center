#!/usr/bin/env node
/**
 * Cross-platform setup script for aicc-marketing
...
if (ws) {
  let total = 0;
  total += copyRecursive(path.join(PLUGIN_ROOT, 'personas'), path.join(ws, '.github', 'ai-ley', 'personas', 'marketing'));
  total += copyRecursive(path.join(PLUGIN_ROOT, 'instructions'), path.join(ws, '.github', 'ai-ley', 'instructions'));
  const agentSrc = path.join(PLUGIN_ROOT, 'agents', 'ailey-marketing.agent.md');
  const agentDest = path.join(ws, '.github', 'agents', 'ailey-marketing.agent.md');
  ...
  if (total > 0) console.log(`  ✅ Installed ${total} Marketing resource(s) into workspace`);
} else {
