#!/usr/bin/env node
/**
 * Cross-platform setup script for aicc-mysql-developer
...
if (ws) {
  let total = 0;
  total += copyRecursive(path.join(PLUGIN_ROOT, 'personas'), path.join(ws, '.github', 'ai-ley', 'personas', 'developer'));
  total += copyRecursive(path.join(PLUGIN_ROOT, 'instructions'), path.join(ws, '.github', 'ai-ley', 'instructions'));
  const agentSrc = path.join(PLUGIN_ROOT, 'agents', 'ailey-mysql-developer.agent.md');
  const agentDest = path.join(ws, '.github', 'agents', 'ailey-mysql-developer.agent.md');
  ...
  if (total > 0) console.log(`  ✅ Installed ${total} MySQL Developer resource(s) into workspace`);
} else {
